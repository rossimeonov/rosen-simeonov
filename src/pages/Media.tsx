  ...
  return (
    <div className="pt-32 bg-white">
      <Helmet>
        ...
        {/* Open Graph / Facebook — Поправено с директен стрингов път към портрета */}
        ...
        <meta name="twitter:image" content="https://www.rosensimeonov.com/assets/hero_portrait.jpg" />
      </Helmet>
      ...
      <section className="py-24 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-6">
           <div className="relative aspect-video bg-slate-100 overflow-hidden group cursor-pointer shadow-2xl rounded-[2rem]">
              {/* Оптимизиран iframe с lazy loading и подобрена сигурност */}
              <iframe 
                className="w-full h-full grayscale opacity-90 hover:opacity-100 transition-all duration-700"
              ...
              ></iframe>
              <div className="absolute bottom-12 left-12 right-12 text-white pointer-events-none hidden md:block">
                 <div className="text-[10px] font-black uppercase tracking-widest bg-brand-600 inline-block px-4 py-2 mb-4 rounded">Видео Репортаж</div>
           ...
           <div className="grid md:grid-cols-2 gap-12">
              {mediaAppearances.map((item, idx) => (
                <motion.div 
                  ...
                  viewport={{ once: true }}
                  className="p-12 border border-slate-100 hover:bg-brand-50 transition-all duration-500 group rounded-[2rem]"
                >
                   ...
                   <p className="text-xl md:text-2xl font-medium text-slate-900 leading-relaxed italic font-serif mb-12 group-hover:translate-x-2 transition-transform line-clamp-5">
                      „{item.description}“
                   </p>
                   <a 
                     href={item.link} 
                     target="_blank" 
                     rel="noopener noreferrer"
                     className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-slate-400 group-hover:text-brand-600 transition-colors mt-auto"
                   >
      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-3xl mx-auto px-6 text-center">
           ...
           <a href="mailto:info@rosensimeonov.com" className="inline-block py-5 px-12 bg-brand-600 text-white font-black text-[10px] uppercase tracking-widest hover:bg-brand-700 transition-all shadow-xl rounded-full">
      ...
      <section className="pb-32 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          {/* Поправено: Премахнат несъществуващия проп variant */}
          <Newsletter />
        </div>
      </section>
    </div>
